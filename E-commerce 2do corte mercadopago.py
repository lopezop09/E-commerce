...
class Database:
    ...
    def get_connection(self):
        """Context manager mejorado para manejar bloqueos con retry automático"""
        conn = None
        ...
        for attempt in range(max_retries):
            try:
                conn = sqlite3.connect(self.db_name, timeout=timeout)
                ...
                if "locked" in str(e).lower():
                    if attempt < max_retries - 1:
                        print(f"🔄 DEBUG: Base de datos bloqueada, reintento {attempt + 1}/{max_retries}")
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            conn.execute("PRAGMA foreign_keys = ON")     # Claves foráneas
    ...
    def diagnosticar_bd(self):
        ...
        try:
            with self.get_connection() as conn:
                ...
                print(f"📊 Tablas existentes: {[t[0] for t in tablas]}")
                
                # Contar registros en tablas clave
                for tabla in ['pedidos', 'items_pedido', 'productos', 'inventario']:
                    count = conn.execute(f"SELECT COUNT(*) FROM {tabla}").fetchone()[0]
                    print(f"   - {tabla}: {count} registros")
                
                # Verificar bloqueos
                print("🔓 Estado de la base de datos: OK")
        ...
        for attempt in range(max_retries):
            try:
                with self.get_connection() as conn:
                    ...
                    print(f"📦 DIAGNÓSTICO - Guardando pedido (intento {attempt + 1}):")
                    print(f"   ID: {pedido['id']}")
                    print(f"   Cliente: {pedido['cliente']}")
                    print(f"   Total: ${pedido['total']:,.2f}")
                    print(f"   Productos: {len(pedido['productos'])}")
                    
                    # 1. VERIFICAR QUE EL PEDIDO NO EXISTA
                    cursor_existente = conn.execute(
                        "SELECT COUNT(*) FROM pedidos WHERE id = ?", 
                        (pedido['id'],)
                    )
                    if cursor_existente.fetchone()[0] > 0:
                        print(f"⚠️  Pedido {pedido['id']} ya existe, omitiendo...")
                        return True
                    
                    # 2. Insertar pedido maestro
                    conn.execute('''
                        INSERT INTO pedidos 
                    ...
                    cursor_items = conn.execute(
                        "SELECT COUNT(*) FROM items_pedido WHERE pedido_id = ?", 
                        (pedido['id'],)
                    )
                    ...
                    if count_pedido > 0 and count_items == items_guardados:
                        ...
                        print(f"   - Items guardados: {count_items}/{items_guardados} ✅")
                        return True
                    else:
                        print(f"❌ INCONSISTENCIA EN DATOS: Pedido {count_pedido}, Items {count_items}/{items_guardados}")
                        if attempt < max_retries - 1:
                ...
                if "locked" in str(e).lower() and attempt < max_retries - 1:
                    print(f"🔒 Base de datos bloqueada, reintento {attempt + 1}/{max_retries}")
...
class PasarelaPago:
    ...
    def completar_pago(self, e):
        """Procesar el pago con integración a MercadoPago"""
        print("DEBUG: Iniciando integración con MercadoPago...")
        ...
        try:
            ...
            success = self.db.guardar_pedido(pedido)
            if not success:
                self.mostrar_error("Error al guardar el pedido. Intenta nuevamente.")
                return
            ...
            if len(items) > 10:
                items = [{
                    "title": f"Compra #{self.order_id} - {len(self.carrito)} productos",
                    "quantity": 1,
                    "currency_id": "COP", 
            ...
            print(f"🔄 Creando preferencia en MercadoPago para ${self.total:,.2f}")
            ...
            try:
                ...
                if preference_response["status"] in [200, 201]:
                    ...
                    # 3. MOSTRAR INTERFAZ DE REDIRECCIÓN
                    self.mostrar_redireccion_mercadopago(payment_url)
                    
                else:
                    error_msg = preference_response.get("response", {}).get("message", "Error desconocido")
                    print(f"❌ Error MercadoPago: {error_msg}")
                    self.mostrar_error(f"Error en MercadoPago: {error_msg}")
                    
            except Exception as mp_error:
                print(f"❌ Error al conectar con MercadoPago: {mp_error}")
                self.mostrar_error(f"Error de conexión: {str(mp_error)}")

        except Exception as error:
            print(f"❌ Error general en pago: {error}")
            self.mostrar_error(f"Error al procesar pago: {str(error)}")

    def mostrar_redireccion_mercadopago(self, payment_url):
        """Mostrar interfaz de redirección a MercadoPago"""
        contenido = ft.Column([
            ...
            ft.Card(
                content=ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(f"ID del pedido: {self.order_id}", size=16),
                        ft.Text(f"Total: ${self.total:,.2f}", size=16),
                        ft.Text(f"Cliente: {self.user['email']}", size=14),
                        ft.Container(height=20),
                        ft.Text("Haz clic en el botón para completar el pago:", size=14),
                        ft.ElevatedButton(
                            "🎯 Ir a MercadoPago para Pagar",
                            on_click=lambda e: webbrowser.open(payment_url),
                            style=ft.ButtonStyle(
                                bgcolor="#00B2FF",
                        ...
                        ft.Container(height=10),
                        ft.Text("O copia esta URL:", size=12, color="gray"),
                        ft.Text(payment_url, size=10, selectable=True, color="lightblue"),
                        ft.Container(height=20),
                        ft.Text("⚠️ No cierres esta ventana hasta completar el pago", 
                               size=12, color="orange", text_align=ft.TextAlign.CENTER),
                        ft.ElevatedButton(
                            "Volver al Inicio",
                            on_click=lambda e: self.finalizar_compra(),
                            style=ft.ButtonStyle(
                                bgcolor="#333333",
                                color="white"
                            )
    ...
    def finalizar_compra(self, e=None):
        """Volver al inicio después del proceso de pago"""
        print("DEBUG: Volviendo al inicio desde pasarela de pago")
        self.page.clean()
        self.volver_callback({})
